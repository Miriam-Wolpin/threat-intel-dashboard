...
async function fetchIntel(ip) {
  try {
    ...
    const qualityRes = await axios.get(
      `https://ipqualityscore.com/api/json/ip/${qualityScoreKey}/${ip}`
    );
    ...
    if (error.response?.status === 429) {
      const service =
        error.config.url.includes('abuseipdb') ? 'AbuseIPDB' : 'IPQualityScore';
      const customError = new Error(`Rate limit exceeded from ${service}`);
